    ...
    while (running() && !interrupt_received) {
		for(x=0;x<32;x++) {
  			...
  			for(y=0;y<32;y++) {
  				...
  				if (theplants[x][y].age>0 && theplants[x][y].age<(int)((PLANTS_LIFE_EXPECTANCY+controldata[16]))) { theplants[x][y].age++; theplants[x][y].energy=theplants[x][y].energy+(int)(PLANTS_ENERGY_BASE_PER_CYCLE+controldata[20]); } // plant grows
  						...
  						if (controldata[1]>0) {
							...
							if (pos>0) {
								...
								switch (available[rand_pos]) { // one individual born radomly
									case 1: if (random_number==1) { specie1[xm][y].age=1; specie1[xm][y].energy=(int)(SPECIE1_ENERGY_BASE+controldata[4]); } break;
									case 2: if (random_number==1) { specie1[xp][y].age=1; specie1[xp][y].energy=(int)(SPECIE1_ENERGY_BASE+controldata[4]); } break;
									case 3: if (random_number==1) { specie1[xm][ym].age=1; specie1[xm][ym].energy=(int)(SPECIE1_ENERGY_BASE+controldata[4]); } break;
									case 4: if (random_number==1) { specie1[x][ym].age=1; specie1[x][ym].energy=(int)(SPECIE1_ENERGY_BASE+controldata[4]); } break;
									case 5: if (random_number==1) { specie1[xp][ym].age=1; specie1[xp][ym].energy=(int)(SPECIE1_ENERGY_BASE+controldata[4]); } break;
									case 6: if (random_number==1) { specie1[xm][yp].age=1; specie1[xm][yp].energy=(int)(SPECIE1_ENERGY_BASE+controldata[4]); } break;
									case 7: if (random_number==1) { specie1[x][yp].age=1; specie1[x][yp].energy=(int)(SPECIE1_ENERGY_BASE+controldata[4]); } break;
									case 8: if (random_number==1) { specie1[xp][yp].age=1; specie1[xp][yp].energy=(int)(SPECIE1_ENERGY_BASE+controldata[4]); } break;
  				...
  				if (specie1[x][y].age==0) { // if theres no individual, new individual will born? (to avoid extintion)
  					if (specie1_neighbours==0) {
  						...
  						if (controldata[2]>0) {
							...
							if (random_number==1) { specie1[x][y].age=1; specie1[x][y].energy=(int)(SPECIE1_ENERGY_BASE+controldata[4]); }
						}
  					}
  				}
  					...
  					if (specie2[x][y].energy>(int)(SPECIE2_ENERGY_TO_REPLICATE+controldata[15])) {
  						...
  						if (controldata[9] >0) {
							...
							if (pos>0) {
								rand_pos=random() % pos;
								switch (available[rand_pos]) { // one individual born radomly
									case 1: if (random_number==1) { specie2[xm][y].age=1; specie2[xm][y].energy=(int)(SPECIE2_ENERGY_BASE+controldata[12]); } break;
									case 2: if (random_number==1) { specie2[xp][y].age=1; specie2[xp][y].energy=(int)(SPECIE2_ENERGY_BASE+controldata[12]); } break;
									case 3: if (random_number==1) { specie2[xm][ym].age=1; specie2[xm][ym].energy=(int)(SPECIE2_ENERGY_BASE+controldata[12]); }break;
									case 4: if (random_number==1) { specie2[x][ym].age=1; specie2[x][ym].energy=(int)(SPECIE2_ENERGY_BASE+controldata[12]); } break;
									case 5: if (random_number==1) { specie2[xp][ym].age=1; specie2[xp][ym].energy=(int)(SPECIE2_ENERGY_BASE+controldata[12]); } break;
									case 6: if (random_number==1) { specie2[xm][yp].age=1; specie2[xm][yp].energy=(int)(SPECIE2_ENERGY_BASE+controldata[12]); } break;
									case 7: if (random_number==1) { specie2[x][yp].age=1; specie2[x][yp].energy=(int)(SPECIE2_ENERGY_BASE+controldata[12]); } break;
									case 8: if (random_number==1) { specie2[xp][yp].age=1; specie2[xp][yp].energy=(int)(SPECIE2_ENERGY_BASE+controldata[12]); } break;
  				...
  				if (specie2[x][y].age==0) { // if theres no individual, new individual will born? (to avoid extintion)
  					if (specie2_neighbours==0) {
  						...
  						if (controldata[10] > 0) {
							...
							if (random_number==1) { specie2[x][y].age=1; specie2[x][y].energy=(int)(SPECIE2_ENERGY_BASE+controldata[12]); }
						}
  					}
  				}
