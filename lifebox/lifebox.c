...
// gcc -o lifebox lifebox.c -lm -lpthread
...
#define PLANTS_LIFE_EXPECTANCY 100
...
#define PLANTS_ENERGY_BASE_PER_CYCLE 5

#define SPECIE1_LIFE_EXPECTANCY 200
#define SPECIE1_RANDOM_BORN_CHANCES 5000
#define SPECIE1_RANDOM_NEARBORN_CHANCES 50
#define SPECIE1_RANDOM_DIE_CHANCES 2
#define SPECIE1_ENERGY_BASE 20
#define SPECIE1_ENERGY_NEEDED_PER_CYCLE 2
#define SPECIE1_MAX_ENERGY_RECOLECTED_PER_CYCLE 20
#define SPECIE1_ENERGY_TO_REPLICATE 5

#define SPECIE2_LIFE_EXPECTANCY 170
#define SPECIE2_RANDOM_BORN_CHANCES 4000
#define SPECIE2_RANDOM_NEARBORN_CHANCES 40
#define SPECIE2_RANDOM_DIE_CHANCES 2
#define SPECIE2_ENERGY_BASE 20
...
while (1) { // Read data from potentiometers loop
	int idata=0;
	char * line = NULL;
	size_t len = 0;
	ssize_t read;
	FILE *controller=fopen("controllerdata","r");
	if (controller != NULL) {
		while ((read = getline(&line, &len, controller)) != -1 && idata<5) {
				len=strlen(line);
				line[len-3] = '\0';
				controldata[idata]=strtod(line,NULL);
				controldata[idata]=controldata[idata]+0.05;
				idata=idata+1;
		}
	}
	if (DEBUG) {
			printf("data[0]:%f | data[1]:%f | data[2]:%f | data[3]:%f | data[4]:%f\n",controldata[0],controldata[1],controldata[2],controldata[3],controldata[4]);
	}
	fclose(controller);
	//usleep(TIME_TO_READ);
	}
	...
	while (1) { // Main loop
		...
		for(x=0;x<32;x++) {
			...
			for(y=0;y<32;y++) {
				...
				if (plantes[x][y].age>0 && plantes[x][y].age<(int)((PLANTS_LIFE_EXPECTANCY*(controldata[4]*2)))) { plantes[x][y].age++; plantes[x][y].energy=plantes[x][y].energy+(int)(PLANTS_ENERGY_BASE_PER_CYCLE*(controldata[4]*2)); } // plant grows
				...
				if (specie1[x][y].age>0) { // if there are an individual alive
					...
					if (specie1[x][y].energy>(int)(SPECIE1_ENERGY_TO_REPLICATE/(controldata[0]*2))) {
						...
						pos=0;
						//srand(time(NULL));
						random_number = random() % (int)(SPECIE1_RANDOM_NEARBORN_CHANCES/(controldata[0]/2));
				...
				if (specie2[x][y].age>0) { // if there are an individual alive
					...
					if (specie2[x][y].energy>(int)(SPECIE2_ENERGY_TO_REPLICATE/(controldata[2]*2))) {
						...
						pos=0;
						//srand(time(NULL));
						random_number = random() % (int)(SPECIE2_RANDOM_NEARBORN_CHANCES/(controldata[2]/2));
				...
				if (specie1[x][y].age==0 && specie2[x][y].age>0) { setPixelColour(x,y,0,1,1); } // only specie2
				if (specie1[x][y].age==0 && specie2[x][y].age==0 && plantes[x][y].age>0) { setPixelColour(x,y,1,1,1); } // only plants
