	...
	while (1) { // bucle principal
		...
		for(x=0;x<32;x++) {
			...
			for(y=0;y<32;y++) {
				...
				if (plantes[x][y].age>0 && plantes[x][y].age<(int)(PLANTS_LIFE_EXPECTANCY*controldata[4]*2)) { plantes[x][y].age++; plantes[x][y].energy=plantes[x][y].energy+PLANTS_ENERGY_BASE_PER_CYCLE; } // plant grows
